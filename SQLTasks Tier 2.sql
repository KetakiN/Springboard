...
with engine.connect() as con:
    rs = con.execute(
" SELECT m.memid AS memberid, m.firstname AS member_firsname,m.surname AS member_lastname, sub.firstname AS recommnederfirst, sub.surname AS recommenderlast 
FROM Members AS m, (SELECT firstname, surname, memid FROM Members) AS sub 
WHERE recommendedby IS NOT NULL AND recommendedby = sub.memid")
    df = pd.DataFrame(rs.fetchall())
    df.columns = rs.keys()
